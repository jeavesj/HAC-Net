...
def _throttle():
    global _LAST_REQ_TS
    now = time.time()
    sleep_for = RATE_LIMIT_SECONDS - (now - _LAST_REQ_TS)
    if sleep_for > 0:
...
def _session_with_retries():
    s = requests.Session()
    retry = Retry(
        total=6,                # total attempts (1 original + 5 retries)
    ...
    s.headers.update({
        'User-Agent': 'HAC-Net/charge-uploader (contact: your_email@domain)',
        'Accept': '*/*',
    })
    return s
...
def _request_with_backoff(session, method, url, **kwargs):
    """
    Throttles BEFORE each call, sends request with retry policy on session,
    and if we still receive 429 with a Retry-After header, we honor it.
    """
    _throttle()
    resp = session.request(method, url, timeout=kwargs.pop('timeout', 120), **kwargs)
    if resp.status_code == 429:
        # If server gives Retry-After, follow it explicitly
...
def _json_object_or_fail(resp, where: str) -> dict:
    """Return a dict from resp.json(); raise if the top-level value is not an object."""
    try:
        j = resp.json()
    except Exception:
        t = (resp.text or "").strip()
        raise RuntimeError(f"{where} did not return JSON (status {resp.status_code}). "
                           f"Body starts: {t[:200]}")
    if isinstance(j, str):
        # Valid JSON can be a string like "OK", but we need an object.
        raise RuntimeError(f"{where} returned a JSON string, not an object: {j[:200]}")
    if not isinstance(j, dict):
        raise RuntimeError(f"{where} returned JSON of type {type(j).__name__}, not an object.")
    return j

""" define a function to predict pkd for a single protein-ligand complex """
def predict_pkd(name, protein_pdb, ligand_file, elements_xml, cnn_params, gcn0_params, gcn1_params, mlp_params, verbose=True, local=False):

  """
  Inputs:
  1) protein_pdb: path to protein pdb file
  2) ligand_file: path to ligand mol2 or pdb file
  ...
  def extract_pocket(protein_pdb, ligand_file):
    print('in extract_pocket')
    
    # read in protein pdb file
    protein =  PandasPdb().read_pdb(protein_pdb)
    print('read protein from pdb')
    ...
    # read in ligand file
    if ligand_file.lower().endswith('mol2'):
        ligand = PandasMol2().read_mol2(ligand_file).df
        if verbose:
            print('read ligand from mol2')
        # define ligand non-H atoms dataframe
        ligand_nonh = ligand[ligand['atom_type'] != 'H'].reset_index(drop=True)
    elif ligand_file.lower().endswith('pdb'):
        ligand_ppdb = PandasPdb().read_pdb(ligand_file)
        ligand = pd.concat([ligand_ppdb.df['ATOM'], ligand_ppdb.df['HETATM']], ignore_index=True)
        if verbose:
            print('read ligand from pdb')
        # define ligand non-H atoms dataframe
        ligand_nonh = ligand[ligand['atom_name'].str.replace(r'[^a-zA-Z]', '', regex=True).str.upper() != 'H'].reset_index(drop=True)
        ligand_nonh = ligand_nonh.rename(columns={'x_coord': 'x', 'y_coord': 'y', 'z_coord': 'z'})
    else:
        raise RuntimeError(f'Ligand file must be type Mol2 or PDB, not {ligand_file}')
  ...
  mol_for_acc2 = next(openbabel.pybel.readfile('pdb', 'pocket.pdb'))
  mol_for_acc2.write('mol2', 'pocket_for_acc2.mol2', overwrite=True)

  """ define function to calculate charges for pocket mol2 file """
  def add_mol2_charges(pocket_mol2, out_mol2='charged_pocket.mol2', timeout_sec=300):
    import io, zipfile, os, tempfile, time

    def _charge_stats(mol2_path):
        try:
            m = next(openbabel.pybel.readfile('mol2', mol2_path))
        except StopIteration:
            return 0, 1.0, 0.0
        qs = [a.partialcharge for a in m.atoms if a.atomicnum > 1]
        if not qs:
            return 0, 1.0, 0.0
        frac_zero = sum(1 for q in qs if abs(q) < 1e-6) / len(qs)
        mean_abs = sum(abs(q) for q in qs) / len(qs)
        return len(qs), frac_zero, mean_abs
    ...
    upj = up.json() if up.headers.get('Content-Type','').startswith('application/json') else {}
    if not (isinstance(upj, dict) and upj.get('success') and upj.get('data')):
        raise RuntimeError(f"ACC2 upload response unexpected: {up.text[:500]}")
    file_hashes = [d.get('fileHash') for d in upj['data'] if isinstance(d, dict)]
    if not file_hashes:
        raise RuntimeError(f"ACC2 upload returned no fileHash. JSON: {up.text[:500]}")

    # ---- setup (keep waters) ----
    setup_body = {
        "fileHashes": file_hashes,
        "settings": {"readHetatm": True, "ignoreWater": False, "permissiveTypes": True}
    }
    st = _request_with_backoff(s, 'POST', 'https://acc2.ncbr.muni.cz/api/v1/charges/setup', json=setup_body)
    if st.status_code >= 400:
        raise RuntimeError(f"ACC2 setup {st.status_code}. Body: {st.text[:500]}")
    stj = st.json() if st.headers.get('Content-Type','').startswith('application/json') else {}
    comp_id = stj.get('data') if isinstance(stj, dict) else None
    if not isinstance(comp_id, str) or not comp_id:
        raise RuntimeError(f"ACC2 setup missing computation_id. JSON: {st.text[:500]}")

    # ---- calculate (eqeq) ----
    calc_body = {
        "fileHashes": [],
        "configs": [{"method": "eqeq", "parameters": ""}],  # parameters MUST be a STRING
        "computation_id": comp_id
    }
    calc = _request_with_backoff(
        s, 'POST', 'https://acc2.ncbr.muni.cz/api/v1/charges/calculate?response_format=none',
        json=calc_body
    )
    if calc.status_code >= 400:
        raise RuntimeError(f"ACC2 calculate {calc.status_code}. Body: {calc.text[:500]}")

    # ---- poll ZIP (gentle polling to avoid 429) ----
    dl_url = f'https://acc2.ncbr.muni.cz/api/v1/files/download/computation/{comp_id}'
    deadline = time.time() + timeout_sec
    zip_bytes = None
    while time.time() < deadline:
        dl = _request_with_backoff(s, 'GET', dl_url)
    ...
    # ---- extract a MOL2 (prefer server MOL2) ----
    wrote = False
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as zf:
        names = zf.namelist()
        mol2_members = [n for n in names if n.lower().endswith('.mol2')]
        if mol2_members:
            with zf.open(mol2_members[0]) as src, open(out_mol2, 'wb') as dst:
                dst.write(src.read())
            wrote = True
        else:
            # Fallback: convert the first convertible format via OBabel
            candidates = [n for n in names if n.lower().endswith(('.sdf', '.sd', '.pdb', '.cif', '.mmcif'))]
            if not candidates:
                raise RuntimeError("ACC2 ZIP contained no convertible structure (no MOL2/SDF/PDB/CIF).")
            member = candidates[0]
            tf = tempfile.NamedTemporaryFile(delete=False)
            with zf.open(member) as src:
                tf.write(src.read())
            tf.flush(); tf.close()
            ext = os.path.splitext(member)[1].lower().lstrip('.')
            ob_fmt = 'sdf' if ext in ('sdf','sd') else ('pdb' if ext=='pdb' else ('cif' if ext in ('cif','mmcif') else None))
            if ob_fmt is None:
                raise RuntimeError(f"Unsupported ACC2 member for conversion: {member}")
            mol = next(openbabel.pybel.readfile(ob_fmt, tf.name))
            mol.write('mol2', out_mol2, overwrite=True)
            wrote = True

    if (not wrote) or (not os.path.exists(out_mol2)) or os.path.getsize(out_mol2) == 0:
        raise RuntimeError("ACC2 produced an empty charged_pocket.mol2")

    # ---- sanity check that charges survived ----
    n_heavy, frac_zero, mean_abs = _charge_stats(out_mol2)
    print(f"[ACC2] charged_pocket.mol2: heavy={n_heavy}, frac_zero={frac_zero:.3f}, mean|q|={mean_abs:.4f}")
  ...
  def add_mol2_charges_local(pocket_mol2, out_mol2='charged_pocket.mol2', model='eqeq'):
    mol = next(openbabel.pybel.readfile('mol2', pocket_mol2))
    try:
      mol.calccharges(model=model)  # 'eqeq' or 'gasteiger'
    except Exception:
      # fallback if eqeq not compiled in
      if model != 'gasteiger':
  ...
  def prepare_data(pocket_mol2_charged, ligand_file, elements_xml):
      ...
      if ligand_file.lower().endswith('mol2'):
        ligand = next(openbabel.pybel.readfile('mol2', ligand_file))
      elif ligand_file.lower().endswith('pdb'):
        ligand = next(openbabel.pybel.readfile('pdb', ligand_file))
      else:
        raise RuntimeError(f'Ligand file must be type Mol2 or PDB, not {ligand_file}')
  ...
  def test_HACNet(cnn_params, gcn0_params, gcn1_params):
      ...
      gcn0_model = GeometricDataParallel(GCN(in_channels=20, gather_width=128, prop_iter=4, dist_cutoff=3.5)).to(device).float()
      ...
      gcn1_model = GeometricDataParallel(GCN(in_channels=20, gather_width=128, prop_iter=4, dist_cutoff=3.5)).to(device).float()
...
class GCN(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        mask = data.edge_attr <= self.dist_cutoff.to(data.edge_attr.device)
        mask = mask.squeeze()
...
def main():
    ...
    parser.add_argument('--gcn1', type=str, default='parameter_files/GCN1_parameters.pt', help='path to other GCN parameter file')
    ...
    for idx, row in idx_df.iterrows():
        ...
        try:
            ...
            if name in done_already:
                print(f'Skipping {name}. Prediction already exists in the output csv.')
                continue
            protein=row['protein']
            ligand=row['ligand']
